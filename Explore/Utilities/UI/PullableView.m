...
@synthesize opened;
@synthesize state;
@synthesize anchoredCenter;
@synthesize isAnchoringEnabled;
...
//                if(state == PullableStateClosed){
//                    minPos = closedCenter.y < anchoredCenter.y ? closedCenter : anchoredCenter;
//                    maxPos = closedCenter.y > anchoredCenter.y ? closedCenter : anchoredCenter;
//                }else if (state == PullableStateHalfOpened){
//                    minPos = anchoredCenter.y < openedCenter.y ? anchoredCenter : openedCenter;
//                    maxPos = anchoredCenter.y > openedCenter.y ? anchoredCenter : openedCenter;
//                }
        ...
        if(isAnchoringEnabled){
            if(axisVelocity < 0){
                if (self.center.y < anchoredCenter.y) {
                    target = openedCenter;
                }else if (self.center.y > anchoredCenter.y){
                    target = anchoredCenter;
                }
            }else{
                if(self.center.y > anchoredCenter.y){
                    target = closedCenter;
            ...
            PullableState toBeState;
            if(CGPointEqualToPoint(target, openedCenter)){
                toBeState = PullableStateOpened;
            }else if(CGPointEqualToPoint(target, anchoredCenter)){
                toBeState = PullableStateHalfOpened;
            }else if(CGPointEqualToPoint(target, closedCenter)){
                toBeState = PullableStateClosed;
            }
            [self setViewState:toBeState animated:YES];
        }else{
...
- (void)handleTap:(UITapGestureRecognizer *)sender {
    ...
    if ([sender state] == UIGestureRecognizerStateEnded) {
        if(isAnchoringEnabled){
            if(state == PullableStateClosed){
                [self setViewState:PullableStateHalfOpened animated:YES];
            }else if(state == PullableStateHalfOpened){
                [self setViewState:PullableStateOpened animated:YES];
            }else if(state == PullableStateOpened){
                [self setViewState:PullableStateHalfOpened animated:YES];
            }
        }else{
...
- (void)setOpened:(BOOL)op animated:(BOOL)anim {
    ...
    if (anim) {
        ...
        if(isAnchoringEnabled){
            if ([delegate respondsToSelector:@selector(pullableView:multiStateChange:)]) {
                [delegate pullableView:self multiStateChange:state];
            }
        }else{
...
- (void)setViewState:(PullableState)pullableState animated:(BOOL)anim {
//    opened = op;
    state = pullableState;
...
//    self.center = opened ? openedCenter : closedCenter;
    ...
    if(state == PullableStateOpened){
        self.center = openedCenter;
        
    }else if (state == PullableStateHalfOpened){
        self.center = anchoredCenter;
        
    }else if(state == PullableStateClosed){
